...
def render_pdf(path: str, outdir: str) -> list[str]:
    prefix=os.path.join(outdir,'page'); subprocess.run(['pdftoppm','-png','-r','220',path,prefix],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL); return sorted(str(p) for p in Path(outdir).glob('page-*.png'))

def preprocess_for_ocr(path: str) -> str:
    img=Image.open(path).convert('RGB'); img.thumbnail((2200,2200)); out=path+'.ocr.png'; img.save(out); return out
