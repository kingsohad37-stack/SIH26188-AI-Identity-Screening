...
def require_user(authorization: str | None) -> tuple[str, Client]:
    token = bearer_token(authorization); client = user_client(token)
    try:
...
def audit(client: Client, screening_id: str, user_id: str, action: str, details=None):
    try: client.table("audit_events").insert({"screening_id": screening_id, "user_id": user_id, "action": action, "details": details or {}}).execute()
    except Exception: pass

@app.get("/health")
def health(): return {"ok": True, "service": "sih26188-screening-api", "model_version": settings.model_version}

def run_analysis(token: str, user_id: str, screening_id: str, doc: dict, supabase: Client):
    local_path = None
    try:
...
def analyze(screening_id: str, background_tasks: BackgroundTasks, authorization: str | None = Header(default=None)):
    token=bearer_token(authorization); user_id,supabase=require_user(authorization)
    ...
    doc=docs[0]
    if screening.get("status")=="processing": return {"screening_id":screening_id,"status":"processing","message":"Analysis already running."}
    ...
    background_tasks.add_task(run_analysis,token,user_id,screening_id,doc,supabase)
    return {"screening_id":screening_id,"status":"processing","message":"Analysis started in background."}
...
def recompute_risk(supabase: Client, screening_id: str, user_id: str):
    ...
    for row in rows:
        if row["status"] in ("passed","failed","suspicious"): evaluated+=1
...
async def face_verify(screening_id: str, reference: UploadFile = File(...), authorization: str | None = Header(default=None)):
    token=bearer_token(authorization); user_id,supabase=require_user(authorization); audit(supabase,screening_id,user_id,"face_verification_started")
    ...
    try:
        ...
        ref_img=cv2.imdecode(np.frombuffer(ref_bytes,np.uint8),cv2.IMREAD_COLOR); doc_bytes=download_user_file(token,docs[0]["storage_path"]); doc_img=cv2.imdecode(np.frombuffer(doc_bytes,np.uint8),cv2.IMREAD_COLOR)
        ...
        risk_score,assessment=recompute_risk(supabase,screening_id,user_id); finding=None
