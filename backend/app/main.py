...
def user_client(token: str) -> Client:
    client: Client = create_client(settings.supabase_url, settings.supabase_publishable_key)
    # The user's JWT is sent to PostgREST so all database operations are evaluated
    # with the authenticated role and the existing RLS policies remain effective.
    client.postgrest.auth(token)
    return client


def require_user(authorization: str | None) -> tuple[str, Client]:
...
def download_user_file(token: str, storage_path: str) -> bytes:
    ...
    try:
        ...
        raise RuntimeError(f"Storage download failed: {type(exc).__name__}") from exc
...
@app.get("/health")
def health():
    return {"ok": True, "service": "sih26188-screening-api", "model_version": settings.model_version}


@app.post("/v1/screenings/{screening_id}/analyze")
def analyze(screening_id: str, authorization: str | None = Header(default=None)):
    token = bearer_token(authorization)
    user_id, supabase = require_user(authorization)
    ...
    doc = docs[0]
    audit(supabase, screening_id, user_id, "analysis_started", {"document_id": doc["id"]})
    ...
    local_path = None
    try:
        blob = download_user_file(token, doc["storage_path"])
        ...
        supabase.table("screening_documents").update({
            ...
            "analysis_metadata": {
                ...
                "forensic_assessment": result["forensic_assessment"],
                "pages_processed": result["pages_processed"],
                "processed_at": result["processed_at"],
            },
        }).eq("id", doc["id"]).eq("user_id", user_id).execute()
        ...
        try:
            ...
            supabase.table("screenings").update({"status": "failed"}).eq("id", screening_id).eq("user_id", user_id).execute()
        except Exception:
            pass
        audit(supabase, screening_id, user_id, "analysis_failed", {"error_type": type(e).__name__})
...
def recompute_risk(supabase: Client, screening_id: str, user_id: str):
    rows = supabase.table("screening_checks").select("check_type,status").eq("screening_id", screening_id).execute().data or []
    score = 0
    evaluated = 0
    weights = {
        "mrz_validation": {"failed": 60, "suspicious": 35, "not_evaluated": 10},
        "image_forensics": {"suspicious": 25, "failed": 40, "not_evaluated": 10},
        "face_verification": {"failed": 45, "suspicious": 25, "not_evaluated": 10},
    }
    for row in rows:
        if row["status"] in ("passed", "failed", "suspicious"):
            evaluated += 1
        score += weights.get(row["check_type"], {}).get(row["status"], 0)
    score = min(100, score)
    assessment = "low_risk" if score < 25 else ("medium_risk" if score < 60 else "high_risk")
    if evaluated == 0:
        assessment = "not_evaluated"
    supabase.table("screenings").update({"risk_score": score, "overall_assessment": assessment}).eq("id", screening_id).eq("user_id", user_id).execute()
...
async def face_verify(screening_id: str, reference: UploadFile = File(...), authorization: str | None = Header(default=None)):
    token = bearer_token(authorization)
    user_id, supabase = require_user(authorization)
    audit(supabase, screening_id, user_id, "face_verification_started")
    ...
    try:
        ...
        doc_bytes = download_user_file(token, docs[0]["storage_path"])
        ...
        check_status = result.get("status", "not_evaluated")
        supabase.table("screening_checks").insert({"screening_id": screening_id, "check_type": "face_verification", "status": check_status, "score": result.get("similarity"), "details": result, "model_version": "opencv-yunet-sface-0.1"}).execute()
        risk_score, assessment = recompute_risk(supabase, screening_id, user_id)
        finding = None
        if check_status == "failed":
            finding = {"screening_id": screening_id, "severity": "high", "code": "FACE_SIMILARITY_MISMATCH", "title": "Face similarity mismatch", "description": "The supplied reference face did not reach the screening similarity threshold. This is a review signal, not proof of impersonation.", "evidence": result}
        elif check_status == "not_evaluated":
            finding = {"screening_id": screening_id, "severity": "warning", "code": "FACE_NOT_EVALUATED", "title": "Face verification not evaluated", "description": result.get("message", "Face verification could not be evaluated."), "evidence": result}
        if finding:
            supabase.table("screening_findings").insert(finding).execute()
        audit(supabase, screening_id, user_id, "face_verification_completed", {"status": check_status})
        return {"screening_id": screening_id, "status": check_status, "result": result, "finding": finding, "risk_score": risk_score, "overall_assessment": assessment}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(500, f"Face verification failed: {type(e).__name__}")
